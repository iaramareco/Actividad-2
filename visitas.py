...
def ingresa_visita(persona):
    ...
    orden = f"""INSERT INTO ingresos_egresos (dni, fechahora_in, destino) 
                VALUES ('{persona.dni}', 
                        '{datetime.datetime.now().replace(microsecond=0).isoformat()}',
                        ''
                );"""
...
def egresa_visita (dni):
    ...
    salida = f"""UPDATE ingresos_egresos (dni, fechahora_out)
        set fechahora_out = '{datetime.datetime.now().replace(microsecond=0).isoformat()}'
        where dni =  p.dni and fechahora_out = ''
    """
    conn.execute(salida)
    conn.commit()
    conn.close()
...
if __name__ == '__main__':
    ...
    p = Persona('55222444', 'Lopez', 'Ana', '02352-456789')
